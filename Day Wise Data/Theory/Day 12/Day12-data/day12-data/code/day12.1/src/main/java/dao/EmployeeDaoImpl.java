...
public class EmployeeDaoImpl implements EmployeeDao {
	...
	public Employee getEmpDetails(int empId) {
		...
		Transaction tx = session.beginTransaction();
		try {
			// Session API : public <T> T get/load(Class<T> pojoClass,Serializable id) ...
			emp = session.load(Employee.class, empId);
		//how to specify : eager woking :select query : non id getter
			emp.getEmail();
			/* if (emp != null) */
				System.out.println("class loaded " + emp.getClass());
			// emp != null => PERSISTENT
		...
		return emp;// un inited proxy => un fetched data 
	}
	...
	public String updateEmpSalary(String email1, double salIncr) {
		...
		try {
			...
			// session.evict(emp); un comment to chk persistent --> detached , cancelling
			// update!
			tx.commit();
