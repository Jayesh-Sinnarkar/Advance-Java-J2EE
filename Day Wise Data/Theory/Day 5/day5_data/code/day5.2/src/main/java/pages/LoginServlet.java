...
//Can u add a parameterized ctro to a servlet ? YES --- BUT WC only invokes def ctor
	public LoginServlet() {
	...
	public void init() throws ServletException {
		...
		try {
			//get db specific cofig from ServletConfig
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			{
				pw.print("<h4>From login servlet....</h4>");
				...
				//pw.flush();un comment to check IllegalStateExc
				...
				//store candidate dao under session scope 
